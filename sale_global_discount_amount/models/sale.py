...
class SaleOrder(models.Model):
    ...
    def _get_line_ids_by_model(self):
        line_ids = super(SaleOrder, self)._get_line_ids_by_model()
...
class SaleOrderLine(models.Model):
    ...
    def _create_discount_lines(self, order):
        ...
        lines_with_price_tax = order.order_line.filtered(
            lambda x: x.price_tax != 0.0)
        res = {}
        for line in lines_with_price_tax:
            taxes = line.tax_id.compute_all(
                line.price_unit, quantity=line.product_uom_qty,
        ...
        for elem in res:
            ...
            self._create_one_discount_line(
                order=order, tax_ids=tax_ids, tax_base_amount=tax_base_amount,
    ...
    def _create_one_discount_line(
        ...
        discount_line = self.env["sale.order.line"].new({
        ...
        discount_line_vals.update({"order_id": order.id,
                                   "tax_id": tax_ids})
