...
""",
    ...
    @api.constrains("promo_type", "discount_type", "discount_amount_currency_id")
    ...
    def _check_valid_total_amount(self, order):
        ...
        return (
            float_compare(self.minimal_amount, amount, precision_digits=precision) < 0
        )
    ...
    def _is_promotion_valid_for_line(self, line):
        ...
        if line.discount and self.multi_rule_strategy == "use_best":
            return (
                float_compare(
                    self.discount_amount, line.discount, precision_digits=precision
                )
                > 0
            )
    ...
    def name_get(self):
        ...
        for record in self:
            if record.rule_type == "coupon":
                ...
                res.append((record.id, "%s (%s)" % (record.name, _("Automatic"))))
            else:
    ...
    def _apply_discount_to_order_lines(self, lines):
        ...
        for order, _lines in lines_by_order.items():
            discount_by_line = {}
            if self.discount_type == "percentage":
                discount_by_line = self._compute_percent_discount_by_lines(order, lines)
            vals = []
            for line in _lines:
                ...
                if self.rule_type == "coupon" and self.discount_type == "percentage":
                    v = {"discount": discount, "coupon_promotion_rule_id": self.id}
                else:
                    v = {"discount": discount, "promotion_rule_ids": [(4, self.id)]}
                vals.append((1, line.id, v))
            if self.discount_type in ("amount_tax_excluded", "amount_tax_included"):
                order_line_discount = self._prepare_order_line_discount(order, lines)
